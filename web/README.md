...
If you're working from the full Mangosoft repository, you can also run `../scripts/run-web.sh [port]` from inside this folder or `./scripts/run-web.sh [port]` from the project root—the helper checks for `python3`, launches the static server from `web/`, and defaults to port 4173 unless you pass a different value.

On macOS you can also double-click `Run Mangosoft.command` in this directory. It opens Terminal, confirms that `python3` is installed, starts the local server, and opens your default browser to the correct URL while keeping the window alive until you press `Ctrl + C`.
